...
**Danbooru 전체 1,190만 장이 들어갑니다.** 이진 양자화와 디스크 그래프로
장당 1,046바이트를 334바이트로 줄였습니다. 근거는
[README](README.md#메모리를-8-gb에-맞춰-줄였습니다)에 있습니다.
...
장당 상주 비용이 SigLIP 334바이트, CLIP 302바이트이므로 이렇게 됩니다.

| 쓰는 모델 | 담을 수 있는 장수 |
|---|---|
| SigLIP + CLIP | 약 840만 장 |
| **SigLIP 하나만** | **약 1,590만 장 (전체 담고 남음)** |

전체를 담을 수 있으므로 `CRAWL_BACKFILL_FLOOR`를 두지 않아도 됩니다.
아래 두 절은 그보다 더 좁은 상자를 쓸 때 참고하십시오.
...
`SAUCEDUST_MAX_INDEXED`에 담을 장수를 두면 거기서 수집을 멈춥니다. 죽는
것이 아니라 멈춥니다.

```bash
SAUCEDUST_MAX_INDEXED=3400000
```
...
level=WARN msg="담을 장수를 정하지 않았습니다. 차면 Qdrant가 죽습니다"
  이 컴퓨터 메모리=8.0 GB  들어갈 만한 장수=4118234
```
...
전체 1,190만 장을 담는 데 실제로 필요한 최소치입니다. 각 항목을 따로
조여서 재 본 값입니다.

| | 최소 | 근거 |
|---|---|---|
| Qdrant | 4.0 GB | 장당 334바이트 실측 |
| PostgreSQL | 0.4 GB | `shared_buffers=96MB`로 384 MB 상자에서 저장소 시험 전부 통과, 실사용 26 MB |
| Python 워커 | 0.5 GB | 두 모델 올려 435 MB 실측 |
| saucedust | 0.2 GB | |
| 운영체제 | 0.4 GB | 최소 설치 |
| **합계** | **약 5.5 GB** | |

**6 GB면 돌고 8 GB면 넉넉합니다.** GPU는 없어도 되고 코어 하나로도
상대 제한을 채웁니다. 다만 여유가 없으면 OOM이 한 번에 오므로 8 GB를
권합니다.

더 조이려면 모델을 하나만 쓰십시오. 워커가 250 MB로 줄고 Qdrant도
장당 302바이트로 줄어듭니다.
