...
| 역할 | 하는 일 | 필요한 것 |
|---|---|---|
| control | 결과 수신, 검색, 대시보드, 죽은 노드 정리 | PostgreSQL, 임베딩 워커 |
| worker | 구간을 임대받아 수집하고 중앙으로 전송 | 임베딩 워커, PostgreSQL 접속 |

작업 노드는 구간 조정을 위해 PostgreSQL에 붙지만, 이미지와 벡터는 HTTP로 중앙에 보냅니다.
그래서 색인과 축소본 저장소는 중앙 노드만 알면 됩니다.

```
worker A ─┐  결과(HTTP)
worker B ─┼─→ control ─→ PostgreSQL + 코드 파일 + 축소본
worker C ─┘      ↑
...
```
작업 노드 A ─┐
작업 노드 B ─┼─ HTTP ─→ 중앙 노드 ─→ PostgreSQL + 코드 파일 + 축소본
작업 노드 C ─┘              ↑
                            └─ 구간 임대 (작업 노드가 직접 조회)
```

**중앙 노드는 한 대**입니다. PostgreSQL, 색인, 축소본을 모두 가집니다.
...
PostgreSQL을 다른 컴퓨터에서도 닿을 수 있게 열어 둡니다.
...
쌓이는 380 GB 중 성격이 다른 두 덩어리가 섞여 있습니다.

| | 크기 | 어떻게 읽히나 | 어디에 |
|---|---|---|---|
| 축소본 | 309 GB | 한 번 쓰고 거의 안 읽음 | **HDD로 충분** |
| 코드 파일, PostgreSQL | 70 GB | 검색할 때마다 읽음 | **SSD여야 함** |

코드 파일은 통째로 훑으므로 순차 읽기입니다. 캐시에 남아 있으면 디스크를
안 보고, 비어 있으면 1 GB쯤을 한 번에 읽습니다. PostgreSQL에서 원본 벡터와
메타데이터를 꺼내는 쪽은 임의 읽기입니다. HDD는 임의 읽기를 초당 100~200회쯤
하고 NVMe는 그 수백 배입니다. 적재는 순차 쓰기라 HDD로도 되지만
**검색이 못 쓰게 됩니다.**
...
PostgreSQL의 자료 위치는 그쪽 설정에서 SSD로 잡으십시오. 코드 파일은
`SAUCEDUST_INDEX_DIR`로 정하고, 비워 두면 `SAUCEDUST_DATA_DIR` 아래에 둡니다.

## 메모리가 색인을 제한하지 않습니다

기본값인 `SAUCEDUST_INDEX=flat`에서는 **메모리가 요구 조건이 아니라
캐시입니다.** 모자라면 죽는 대신 느려집니다.

이진 코드만 파일에 두고 통째로 훑습니다. 파일을 주소 공간에 걸어 두므로
힙에 올라가는 것이 없습니다. 커널이 페이지 캐시로 들고 있다가 메모리가
모자라면 알아서 버립니다.

1,190만 장 그대로 만들어 재 본 값입니다.

| | 값 |
|---|---|
| 코드 파일 | 1.06 GiB (768차원 기준 장당 96바이트) |
| Go 힙 사용량 | 2.5 MiB |
| 2코어에서 한 질의 | 41밀리초 |
| 1코어에서 한 질의 | 80밀리초 |

페이지 캐시가 비어 있으면 파일을 다시 읽어야 합니다. SATA SSD 기준으로
1.06 GiB에 2초쯤입니다. **질의마다가 아니라 다시 켠 뒤 한 번**이고, 그
뒤로는 캐시에 남습니다.

그래프를 안 타므로 빠뜨리는 점이 없습니다. 30장으로 끝에서 끝까지
돌려 Qdrant와 견주니 답과 점수가 소수점 일곱 자리까지 같았습니다.

### Qdrant를 쓰려면

이미 Qdrant에 자료가 있으면 `SAUCEDUST_INDEX=qdrant`로 두십시오. 그쪽은
장수에 비례하는 몫을 반드시 램에 붙들고, 모자라면 죽습니다. 8 GB에서
SigLIP 하나로 약 1,289만 장, 두 모델이면 약 677만 장입니다.

바꿔 옮기려면 `saucedust rebuild`가 PostgreSQL의 벡터로 색인을 다시
세웁니다. 코드 파일은 언제든 다시 만들 수 있는 파생물이라, 지우고 다시
만들어도 잃는 것이 없습니다.

**한 폴더는 한 프로세스만 씁니다.** `control`이 도는 중에 `reembed`를
돌리면 잠금에 걸려 거절합니다. 두 프로세스가 각자 제 자리 수를 세고 그
자리에 쓰면 서로를 덮기 때문입니다. `stats`처럼 읽기만 하는 명령은
함께 쥘 수 있습니다.

아래 두 절은 Qdrant를 쓰면서 상자가 좁을 때 참고하십시오.
...
납작한 색인에서는 필요 없습니다. 시작할 때 이렇게 알려 줍니다.

```
level=INFO msg="납작한 색인이라 메모리가 담을 장수를 제한하지 않습니다"
  장당 디스크 바이트=176  1,190만 장이면=2.0 GB
```

Qdrant를 쓸 때만 읽으십시오. `SAUCEDUST_MAX_INDEXED`에 담을 장수를 두면
거기서 수집을 멈춥니다. 죽는 것이 아니라 멈춥니다.

```bash
SAUCEDUST_MAX_INDEXED=6700000
```

두지 않으면 시작할 때 이 컴퓨터에 몇 장이 들어가는지 알려 줍니다. 가상
기계나 컨테이너 안이면 cgroup 상한을 봅니다.

```
level=WARN msg="담을 장수를 정하지 않았습니다. 차면 Qdrant가 죽습니다"
  이 컴퓨터 메모리=8.0 GB  들어갈 만한 장수=6768920
```

모든 노드가 **같은 값**을 씁니다. 한계는 중앙의 Qdrant 것이지 작업 노드
것이 아니라서, 노드마다 제 메모리를 보고 판단하면 틀립니다. 판단은 공유
저장소에 든 전체 장수로 합니다.

두 군데에서 지킵니다. 수집하는 쪽은 구간을 받기 전에 보고, **중앙은 자료가
들어올 때 봅니다.** 수집 쪽만 두면 설정을 빠뜨린 노드나 API를 직접 부르는
쪽이 그대로 지나갑니다. 찬 상태에서 자료를 보내면 507로 거절하고, 받은
쪽은 다시 보내지 않고 멈춥니다.

**찬다고 검색이 멈추지는 않습니다.** 중앙 노드는 같은 프로세스에서 검색과
대시보드를 함께 맡으므로, 수집만 접고 나머지는 그대로 답합니다. 상한을
올리면 다시 띄우지 않아도 이어서 돕니다.

수집 중에 상대 사이트가 멈추면 실패한 구간을 1분부터 두 배씩 쉬었다가
다시 잡습니다. 쉬지 않고 다시 잡으면 몇 초 만에 시도 횟수를 채워 그 ID
대역이 영영 빠집니다.

**찬 동안에 모으던 것을 잃지도 않습니다.** 색인이 차서 못 넣은 구간과
재시도는 시도 횟수를 쓰지 않고 그대로 돌려놓습니다. 실패로 적으면 다섯
번 만에 죽어서, 나중에 자리가 생겨도 그 게시물이 영영 빠집니다.
...
이진 양자화는 30만 점 기준 하한을 450 MB에서 300 MB로 낮췄습니다. 2배지
8배가 아닙니다. int8을 8분의 1로 줄여도 HNSW 그래프가 장당 400바이트쯤을
그대로 차지하기 때문입니다. **정확도는 재지 않았으므로 기본값으로 두지
않았습니다.** 실제 그림으로 확인하기 전에는 쓰지 마십시오.

## 가장 작은 상자

전체 1,190만 장을 담는 데 필요한 최소치입니다. 항목마다 따로 조여서
재 본 값입니다.

| | 최소 | 근거 |
|---|---|---|
| 색인 | 0 | 걸어 두기만 하므로 반드시 램에 있어야 하는 몫이 없습니다 |
| PostgreSQL | 0.4 GB | `shared_buffers=96MB`로 384 MB 상자에서 저장소 시험 전부 통과, 실사용 26 MB |
| Python 워커 | 0.5 GB | 두 모델 올려 435 MB 실측. 하나면 250 MB |
| saucedust | 0.2 GB | |
| 운영체제 | 0.4 GB | 최소 설치 |
| **합계** | **약 1.5 GB** | |

여기에 코드 파일을 캐시로 들고 있을 만큼을 더하면 빨라집니다.

| 메모리 | 무슨 일이 벌어지나 | 한 질의 |
|---|---|---|
| 2 GB | 캐시가 거의 없어 매번 디스크에서 읽습니다 | 2초쯤 |
| 3 GB | 절반쯤 캐시에 남습니다 | 1초쯤 |
| **4 GB** | **SigLIP 코드 1.06 GiB가 다 남습니다** | **41밀리초** |
| 6 GB | 두 모델 코드가 다 남습니다 | 41밀리초 |

**2 GB에서도 돌고 4 GB면 빠릅니다.** 어느 쪽이든 죽지 않습니다. GPU는
없어도 되고 코어 하나로도 상대 제한을 채웁니다.

## Proxmox 가상 기계 권장 사양

역할을 나눌 때와 한 대로 할 때가 다릅니다.

**같은 호스트에 두 대를 두면 수집이 빨라지지 않습니다.** Danbooru가 IP
단위로 제한하는데 같은 공인 IP로 나가기 때문입니다. 수집 노드를 늘리는
것은 회선이 다를 때만 뜻이 있습니다.

### 중앙 노드

| | 값 | 왜 |
|---|---|---|
| 메모리 | 4 GB | 2 GB로도 돕니다. 4 GB면 코드가 캐시에 남아 빠릅니다 |
| vCPU | 2 | 질의 임베딩 한 장에 80밀리초, 훑기에 41밀리초입니다 |
| CPU 종류 | `host` | torch가 AVX를 씁니다. `kvm64`면 느려집니다 |
| 디스크 컨트롤러 | VirtIO SCSI single | |
| SSD | 100 GB, `cache=none` | 시스템과 PostgreSQL과 코드 파일 |
| HDD | 350 GB | 축소본만 |
| 네트워크 | VirtIO | |
| GPU | 필요 없습니다 | 질의 한 장에 GPU를 넘길 이유가 없습니다 |

`cache=none`이 아니면 호스트가 갑자기 꺼졌을 때 PostgreSQL이 지켰다고 믿은
쓰기가 사라집니다.

1,190만 장을 채웠을 때 SSD에 들어가는 것입니다.

| | SigLIP 하나 | 모델 둘 |
|---|---|---|
| 코드 파일 | 1.2 GB | 2.1 GB |
| PostgreSQL 벡터 백업 | 39 GB | 62 GB |
| PostgreSQL 메타데이터 | 9 GB | 9 GB |
| 시스템, 가상 환경, torch, 가중치 | 20 GB | 20 GB |
| **합계** | **약 70 GB** | **약 95 GB** |

축소본 309 GB는 HDD로 갑니다.

Qdrant를 쓰면 여기에 벡터 사본 48 GB가 더 붙고 메모리 요건이 생깁니다.

### 수집 노드

| | 값 | 왜 |
|---|---|---|
| 메모리 | 2 GB | 운영체제 0.7, 워커 0.5, saucedust 0.2, 나머지가 버퍼 |
| vCPU | 2 | CPU 임베딩이 1스레드로 초당 12.6장, 상대 제한이 초당 5장입니다 |
| CPU 종류 | `host` | 위와 같습니다 |
| SSD | 32 GB | 가상 환경과 가중치가 약 20 GB |
| HDD | 필요 없습니다 | 이미지도 축소본도 남지 않습니다 |
| GPU | 필요 없습니다 | 1코어로도 상대 제한을 두 배 넘깁니다 |
...
동시성을 낮추는 이유가 있습니다. 기본값 16에 이미지 한 장 상한이 32 MB라,
최악의 경우 512 MB가 내려받는 중인 상태로 메모리에 떠 있습니다. 2 GB
상자에서는 그것만으로 물립니다. 어차피 상대 제한이 초당 5장이라 4로
낮춰도 안 느려집니다.

`models.json`에서 `clip-b32`를 지우면 코드 파일과 워커가 절반쯤으로
줄어듭니다. CLIP은 "비슷한 분위기의 그림"을 찾는 용도라 출처 찾기가
목적이면 빼도 됩니다.
...
**메모리는 4 GB면 됩니다.** 납작한 색인은 코드 파일을 걸어 두기만 하므로
반드시 램에 있어야 하는 몫이 없습니다. 1,190만 장을 넣고 재니 힙이
2.5 MB였습니다. 남는 메모리는 캐시로 쓰여 빠른지 느린지만 정합니다.

풍선(ballooning)은 Qdrant를 쓸 때만 꺼 두십시오. Qdrant는 상한 아래로
내려가면 뜨지 않고 그냥 죽습니다. 납작한 색인은 회수해 가도 느려질 뿐입니다.
